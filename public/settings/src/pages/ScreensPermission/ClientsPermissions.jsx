...
export default function ClientsPermissions() {
  const navigate = useNavigate();
  const { updateScreenSettings } = useInvoiceSettings();
  const { customerShow, setCustomerShow } = useScreenSettingsPreferenceLogic();

  const settings = [
    {
      label: "السماح بعرض شاشة الزبائن",
      value: customerShow,
      setter: setCustomerShow,
    },
  ];
