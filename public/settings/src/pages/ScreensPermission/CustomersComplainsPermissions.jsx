...
export default function CustomersComplainsPermissions() {
  const navigate = useNavigate();
  const { updateScreenSettings } = useInvoiceSettings();
  const { customerShowMessages, setCustomerShowMessages } =
    useScreenSettingsPreferenceLogic();

  const settings = [
    {
      label: "السماح بعرض رسائل الزبائن ",
      value: customerShowMessages,
      setter: setCustomerShowMessages,
    },
  ];
  ...
  useEffect(() => {
    const screensSettingSubmit = () => {
      const payload = {
        customerShowMessages: customerShowMessages,
      };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 lg:p-8 flex flex-col items-center">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">
        إعدادات صلاحيات شاشة شكاوى الزبائن
      </h1>
