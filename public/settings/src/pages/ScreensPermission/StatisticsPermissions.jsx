...
export default function StatisticsPermissions() {
  const navigate = useNavigate();
  const { updateScreenSettings } = useInvoiceSettings();
  const { statisticsShow, setStatisticsShow } =
    useScreenSettingsPreferenceLogic();

  const settings = [
    {
      label: "السماح بعرض شاشة الاحصائيات",
      value: statisticsShow,
      setter: setStatisticsShow,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 lg:p-8 flex flex-col items-center">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">
        إعدادات صلاحيات شاشة الاحصائيات
      </h1>
