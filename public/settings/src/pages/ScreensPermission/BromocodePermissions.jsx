...
export default function BromocodePermissions() {
  const navigate = useNavigate();
  const { updateScreenSettings } = useInvoiceSettings();
  const { bromocodeShow, setBromocodeShow } =
    useScreenSettingsPreferenceLogic();

  const settings = [
    {
      label: "السماح بعرض شاشة البروموكود",
      value: bromocodeShow,
      setter: setBromocodeShow,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 lg:p-8 flex flex-col items-center">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">
        إعدادات صلاحيات شاشة البروموكود
      </h1>
