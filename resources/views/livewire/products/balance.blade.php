                ...
                <table class="min-w-full divide-y text-center divide-gray-200 text-sm text-right text-gray-700 bg-white">
                    <thead class="bg-blue-200 text-gray-700 text-xs font-semibold uppercase">
                        <tr>
                            <th colspan="10" class="px-3 py-3 bg-blue-50">
                                <div class="flex justify-between items-center gap-2 flex-wrap">
                                    ...
                                    <div>
                                        <button wire:click="$set('barcode', ''); $set('search', '')"
                        ...
                        <tr>
                            ...
                            <th class="px-3 py-2 border border-gray-200">الرصيد الحالي</th>
                            <th class="px-3 py-2 border border-gray-200">التكلفة</th>
                            <th class="px-3 py-2 border border-gray-200">سعر البيع</th>
                            <th class="px-3 py-2 border border-gray-200">إجمالي التكلفة</th>
                            <th class="px-3 py-2 border border-gray-200">إجمالي سعر البيع</th>
                    ...
                    <tbody>
                        @forelse($products as $index => $product)
                             <tr wire:key="product-{{ $product->id }}" class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2 border border-gray-200">{{ number_format($product->stock, 2) }}
                                </td>
                                <td class="px-3 py-2 border border-gray-200">{{ number_format($product->cost, 2) }}</td>
                                <td class="px-3 py-2 border border-gray-200">{{ number_format($product->price, 2) }}
                                </td>
                                <td class="px-3 py-2 border border-gray-200">
                                    {{ number_format($product->total_cost, 2) }}</td>
                                <td class="px-3 py-2 border border-gray-200">
                                    {{ number_format($product->total_price, 2) }}</td>
                                <td class="px-3 py-2 border border-gray-200">
            ...
            <div class="p-4 bg-gray-50 border-b">
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    <div>
                        ...
                        <span
                            class="block mt-1">{{ $selectedProduct->productUnits->first()?->unit?->name ?? '--' }}</span>
                    </div>
                    <div>
                        <span class="font-semibold">التكلفة:</span>
                        <span
                            class="block mt-1">{{ number_format($selectedProduct->productUnits->first()?->price ?? 0, 2) }}
                            ج.م</span>
                    </div>
                    <div>
                        <span class="font-semibold">سعر البيع:</span>
                        <span
                            class="block mt-1">{{ number_format($selectedProduct->productUnits->first()?->sallprice ?? 0, 2) }}
                            ج.م</span>
                    </div>
                </div>
            </div>
                            ...
                            @forelse($selectedProduct->warehouseProducts as $wp)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        {{ number_format($wp->quantity * ($selectedProduct->productUnits->first()?->price ?? 0), 2) }}
                                        ج.م</td>
                                    <td class="px-4 py-2 text-center">
                                        {{ number_format($wp->quantity * ($selectedProduct->productUnits->first()?->sallprice ?? 0), 2) }}
                                        ج.م</td>
                                </tr>
                            @empty
                                <tr>
