                        ...
                        @forelse($invoices as $index => $invoice)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2 border border-gray-200 font-bold text-green-600">
                                    {{ number_format($invoice->total ?? 0, 2) }} ج.م
                                </td>

                                <td class="px-3 py-2 border border-gray-200">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-sm">{{ $invoice->created_at ? $invoice->created_at->format('Y-m-d') : $invoice->date ?? '--' }}</span>
                                ...
                                <td class="px-3 py-2 border border-gray-200 text-center">
                                    <div class="inline-flex items-center justify-center gap-1">
                                        <!-- زر عرض الفاتورة -->
                                        <button type="button" class="btn btn-outline-info p-1" data-bs-toggle="modal"
                                            data-bs-target="#invoiceModal"
                                            onclick="loadInvoiceDetails('{{ $invoice->id }}',
                                                    '{{ $invoice->id }}',
                                                    '{{ $invoice->address ?? '' }}',
                                                    '{{ $invoice->products_json  ?? '' }}',
                                                    '{{ $invoice->cashier->name ?? '' }}',
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">المنتج
                                        </th>
                                        <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">
                                            الكمية</th>
                                        <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">السعر
                                        </th>
                                        <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase">
                                            الإجمالي</th>
                                    </tr>
                                ...
                                <tfoot class="bg-gray-50">
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-center font-bold text-success" id="modalTotal">0.00 ريال
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-center font-bold text-blue-600 text-lg"
                                            id="modalFinalTotal">0.00 ريال</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
@section('scripts')
    <script>
        function loadInvoiceDetails(invoiceId, invoiceNumber, address, products, cashier, total, dateTime) {
            ...
            document.getElementById('modalTotal').textContent = total + ' ج.م';
            ...
            document.getElementById('modalFinalTotal').textContent = finalTotal.toFixed(2) + 'ج.م';
            console.log(products);
            // محاكاة جلب بيانات المنتجات
            simulateProductsLoading(products);
        }

        function simulateProductsLoading(products) {
            ...
            <tr>
                ...
                <td class="px-4 py-2 text-center">${product.price} ج.م</td>
                <td class="px-4 py-2 text-center">${product.total} ج.م</td>
            </tr>
        `;
            });
